...
const CreateCompany = () => {
    ...
    const handleCreateCompany = async (e: React.FormEvent) => {
        ...
        try {
            ...
            const userResponse = await axios.post('http://localhost:5198/api/auth/register', {
                Email: email,
                Password: password,
                FirstName: firstName,
                LastName: lastName
            ...
            // Extract the user ID from the response (assuming the API returns the user ID)
            const userId = userResponse.data.userId;
    
            // Then, create the company using the user ID as CEOId
            await axios.post('http://localhost:5198/api/company/create', {
                Name: companyName,
                Industry: industry,
                CEOId: userId // Send the user ID as CEOId
            });
    
            setSuccess('Company and user created successfully!');
        } catch (err) {
            setError('Failed to create company or user');
        }
    };
